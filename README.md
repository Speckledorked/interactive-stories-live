...
- **Fog of War**: hidden factions/NPCs/locations, GM-only notes, and exact simulation numbers never reach player-facing text or prompts — enforced at the query layer, not just the UI. Audited route by route and currently accurate; it is however enforced by hand at each call site rather than by a shared mechanism, which is tracked as `#94` rather than claimed as solved.
...
| Faction simulation (goals/collapse/succession/territory) | 4 | Goal-driven stat deltas, banded reassessment, collapse → absorption or remnant succession, real territory reassignment. Deepest subsystem in the codebase.  **Marked down for succession specifically**: `leadershipTick.ts` is the only tick module that exports no pure `decide*` function — all its logic is inline in the DB handler — and `tickFactionLeadership` appears in zero test files. Every other rule in this row has a tested pure form; the one named in the title does not.|
...
| World tick orchestration | 4 | Nine deterministic handlers, genuinely sequenced same-tick dependencies, zero AI calls — all verified. Marked down because **the sequencing is the claim and no test executes it**: `runWorldTick` appears in zero test files, so the handler order lives in a long comment and is enforced by nothing. Reorder `TICK_HANDLERS` and the suite stays green. |
...
| Quest identity/gating (does anything react to who gave it, or to FAILED/ABANDONED) | 3 | Partly fixed (`#45`, `#75`): `givenBy` now resolves to a real NPC/faction FK and quests carry a stable `objectiveKey`, so a quest can finally reach its giver and be referred to by other content. Corruption gates acquisition. Still a 3, not higher: FAILED/ABANDONED remain inert — what failure *costs* is the open design question, and the structure to answer it now exists. |
...
| **Outcome-band adherence (does the narration obey the roll?)** | 1 | The engine rolls, the prompt says the band is BINDING, and **nothing checks that the prose complies.** Grepped for it: there is no post-hoc comparison of `scene_text` against `strongHit`/`weakHit`/`miss` anywhere. A model can narrate a triumph on a MISS and the only artifact that disagrees is a receipt in a panel the player has to open. The dice being server-side is real; "binding" is currently a request, not a constraint. |
| **Fog-of-war enforcement mechanism** | 2 | The *outcome* is right — audited all 57 campaign routes: 55 carry a membership check, and the only route returning factions without discovery filtering is admin-gated, correctly. But there is **no shared mechanism**: 23 hand-written `isDiscovered`/`isHidden` clauses across 11 route files, no helper, no policy layer, and no test. This is the exact duplicated-rule pattern this ledger keeps fixing, sitting at the security layer where drift costs a leak instead of a wrong number. A new route that returns NPCs and forgets is caught by nothing. |
| **API route test coverage** | 1 | **6 test files for 93 routes.** Every claim about authorization, fog of war, and membership gating lives in the ~87 untested ones. The engine underneath is well covered; the layer users actually reach is not. |
| **Auth / session** | 2 | Standard `jsonwebtoken` HS256, secret required from env, `verifyAuth`/`getUser` used consistently. But: **30-day expiry with no revocation path** — no token-version column, denylist, or logout-everywhere — so a leaked token is valid for a month, and `src/lib/auth.ts` has **zero tests**. Adequate for a closed beta, not for the public API in `#27`. |
...
are folded in below. `#22` (de-jargon), `#23` (surface multiplayer) and `#45`/`#75` (structured quest objectives and a real quest-giver FK) shipped — see Shipped below. What a FAILED or ABANDONED quest should *cost* is still open; the structure to answer it now exists.

- [ ] **#93 Nothing enforces the outcome band on the narration** — P1, and the most load-bearing gap found in the scorecard re-audit. The engine rolls server-side and the prompt calls the band BINDING; no code compares the returned `scene_text` against `strongHit`/`weakHit`/`miss`. A model narrating a clean success on a MISS is invisible to the system, which undercuts the headline claim that outcomes constrain the fiction rather than being suggested to it. Cheap first step: a post-resolution check that flags a mismatch into the transparency receipt and the AI-consistency metric (the plumbing for both now exists) rather than trying to rewrite the prose.
- [ ] **#94 Fog of war has no shared enforcement mechanism** — P1. The rule is currently 23 hand-written `isDiscovered`/`isHidden` clauses across 11 route files. Audited today and the *current* state is correct — 55 of 57 campaign routes carry a membership check, and the one route returning factions unfiltered is admin-gated on purpose — but correctness rests on every future route author remembering. This is the duplicated-rule pattern the rest of this ledger keeps eliminating, at the layer where drift is a data leak. Wants a shared scope helper (`whereVisibleTo(role)`) plus a structural test asserting that any route selecting a fog-gated model either uses it or is admin-only.
- [ ] **#95 The API layer is effectively untested — 6 test files for 93 routes** — P1. The simulation engine is thoroughly covered; the surface users actually reach is not, and that is where authorization, membership and fog-of-war decisions live. Not a request for 93 test files: the priority is the routes that write state or gate visibility, starting with `scene`, `resolve-scene`, `end-scene`, `members`, `invites` and the entity CRUD routes.
- [ ] **#96 `runWorldTick`'s handler order is enforced by nothing** — P2. The same-turn dependency chain is the substance of the tick's design and is documented in a 25-line comment; `runWorldTick` appears in zero test files, so reordering `TICK_HANDLERS` breaks the simulation silently and keeps the suite green. Wants one integration test asserting the sequenced dependencies actually hold (a collapse promoting a successor in the same tick, an ambition skipping a faction that just went to war).
- [ ] **#97 Faction succession is the one simulation rule with no pure form and no test** — P2. Every other tick module exports testable `decide*` functions; `leadershipTick.ts` exports only its DB handler, and `tickFactionLeadership` appears in zero test files. Extract the decision and test it, matching the shape the other eight handlers already use.
- [ ] **#98 30-day JWTs with no revocation path** — P2 now, P1 before `#27` (public API). No token-version column, denylist, or logout-everywhere, so a leaked token stays valid for a month; `src/lib/auth.ts` also has zero tests. Fine for a closed beta, not for issuing developer credentials.
