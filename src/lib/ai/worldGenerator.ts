...
// Generated by worldExtras.ts's second-stage call, NOT here — this
// call's JSON response is already at its token budget (see the doc
// comment on generateWorldFromTemplate below); cramming more required
// fields into the same response risks truncating it entirely, which
// would silently zero out factions/capabilities/fronts too, not just
// these. Types live here anyway since worldExtras already imports
// GeneratedCapability/GeneratedStatLabels from this module.
...
- stat_labels: rename all 5 stats to fit this world's own vocabulary (e.g. a cultivation setting might call "weird" something like "Essence Sense"; a hard sci-fi setting might not use mystical language at all).${loreDigest ? ' If the canon lore names a real stat/attribute system for this universe (a progression-fantasy series\' own attributes, a game-like power system, etc.), REUSE those real canon names for as many of the 5 slots as reasonably map to what each stat measures — do not invent a fictional-sounding replacement when canon already names one. Canon may name fewer than 5 (or more) — map what fits and invent only the remaining slot(s) to round out the mechanics.' : ''} Keep each label 1-3 words, in-fiction, never the literal PbtA name. The underlying meaning (what each stat measures) must stay the same — only the name and flavor change
