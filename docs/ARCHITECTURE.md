...
1. **Decide whether to enable `integrity-autofix.yml`'s `schedule` trigger
   ...
   Autonomous code-fix pipeline row for what that run proved, and the Fix
   Log for the 8 pipeline-plumbing defects fixing it surfaced along the
