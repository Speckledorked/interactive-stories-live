...
### Phase 6.5 — Fog of War ✅ (core; location discovery deferred)
- [x] Structural secrecy: `gmNotes` is now stripped from every player-facing API response (campaign, factions, npcs, clocks) via a shared `redactGmNotesList()` helper — enforced at the API layer regardless of what the AI does with its prompt. Admins still see it
- [x] Real discovery flags: `NPC.isDiscovered` / `Faction.isDiscovered` (default `true`, so nothing existing changes behavior). `applyWorldUpdates()` takes a `sceneOrigin` flag — a live scene the players witnessed flips an entity to discovered on mention, but an offscreen background tick event never does, so the simulation can move a hidden faction or NPC around without the party learning about it for free. Manual toggles added to the admin panel for both
- [x] Both AI world-summary builders (`buildWorldSummaryForAI` and the token-optimized `buildOptimizedWorldSummary`) filter factions/NPCs by `isDiscovered` before they reach the LLM, null out a location's `owner_faction_id` if the owner is undiscovered, and drop wars where either side is undiscovered
...
- [ ] `Location.isDiscovered` — pre-existing field, still a no-op nobody ever sets false; discoverable geography (as opposed to discoverable people/organizations) is out of scope for this pass
