...
- API route test coverage is targeted, not broad — 104 routes, 72
  ...
  checkout, stuck-scene recovery, campaign-scoped user blocking). ~32
...
2. **Keep broadening API route test coverage.** #93, #134, and #135 have
   ...
   backfill), health-check, and tutorial tiers (see the Scorecard row); ~32 routes remain untested —
   admin reads, lore/wiki CRUD, and internal
...
- **API route test coverage** — targeted coverage exists for the
  ...
  admin/GM-tools, health-check, and tutorial tiers (see the Scorecard) — but ~32 lower-risk routes
