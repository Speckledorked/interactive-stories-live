...
**`TavernCard` exists but ~15 sites bypass it with the raw class string (#119)** — `tavern/ui.tsx` already exports `TavernCard`, exactly `rounded-xl bg-gradient-to-br from-tavern-800/70 to-tavern-900/70 border border-ember-900/30 shadow-lg shadow-black/30`, for this one purpose. 15 files hand-roll that identical string inline instead of importing it, including the 6 sidebar panels this very pass split out of `story/page.tsx` (`CharacterSelectorPanel`, `TurnOrderPanel`, `MapViewerPanel`, `SceneChatPanel`, `ActiveClocksPanel`, `RecentTimelinePanel`) — preserved verbatim from the original inline JSX on purpose, since swapping them to `TavernCard` mid-split would have mixed a structural move with a styling change in the same diff. Not fixed here: this pass's mandate was moving/deduplicating existing patterns, not adopting an existing-but-unused one across 15 call sites, which is a larger, separately-verifiable change.
- *Evidence:* `src/components/tavern/ui.tsx` (`TavernCard`, line 25); bypassing sites include `src/app/campaigns/[id]/wiki/page.tsx`, `src/app/settings/page.tsx`, `src/components/turns/TurnTracker.tsx`, `src/components/scene/VisualTimeline.tsx`, `src/app/tutorial/page.tsx`, `src/app/help/page.tsx`, `src/app/admin/analytics/page.tsx`, and the 6 new scene panel components.
- *Scope:* maintainability only — no visual difference, since the bypassing sites use the identical class string.
- *Suggested fix:* a follow-up pass swapping each bypass site to `<TavernCard>`, verified with the same before/after screenshot method this pass used — mechanical, but 15 call sites' worth of diff and verification, not a one-line change.

**`FieldHelp`/`StatInfo` popover duplication, considered and rejected (#120)** — `src/components/ui/field-help.tsx` (`FieldHelp`) and `src/components/character/StatBar.tsx` (`StatInfo`) are the same shape: a `?`/`ⓘ` button toggling an absolutely-positioned popover, closed by `useEscapeKey` plus an outside-pointer-down effect, tracked with `useId` for `aria-controls`. Considered and rejected rather than merged: `FieldHelp` renders the newer "myth" theme's classes and a structured `what`/`whoItAffects`/`whenToUse` content shape and closes on `mousedown` only; `StatInfo` renders the older "tavern/ember" theme's classes, a single description string, and closes on both `mousedown` and `touchstart` for mobile. A shared component would need a theme prop, a content-shape union, and a configurable pointer-event set — more surface than the ~25 lines of genuine overlap it would save.
- *Evidence:* `src/components/ui/field-help.tsx`, `src/components/character/StatBar.tsx` (`StatInfo`, line 19).
- *Scope:* maintainability only — no behavior difference either popover exhibits today.

**`SimpleXCard`/`ReportContentModal` near-duplicate modal shell, considered and rejected (#121)** — both are self-contained "button that opens a centered `fixed inset-0 bg-black/80 z-50` modal, submits via `authenticatedFetch`, shows a ✅ success state, auto-closes on a timer" component, with the Cancel button styled identically in both. Considered and rejected: `SimpleXCard` deliberately has no validation and no required field — the product decision documented in its own copy is "use this tool freely, no explanation needed" — while `ReportContentModal` requires a non-empty reason and renders an inline validation error. The two dropdowns are domain-specific (safety trigger categories vs. content-type categories) rather than the same list twice. Unifying the chrome would still leave the required-vs-optional field as a prop, which is most of what differs.
- *Evidence:* `src/components/safety/SimpleXCard.tsx`, `src/components/safety/ReportContentModal.tsx`.
- *Scope:* maintainability only — no behavior difference either modal exhibits today.

**Two independent slide-in-drawer implementations, considered and rejected (#122)** — `AdminNav.tsx`'s mobile drawer (`drawerOpen`, admin-section nav only, myth-theme classes only, no entrance animation) and `TavernMobileMenu.tsx` (the app-wide hamburger menu, dual tavern/myth theme variants, `animate-slide-up`, its own link list plus Log Out) are both a "left-edge, `w-72 max-w-[85vw]`, backdrop-click-to-close, X button" overlay, and both now close on Escape via the same `useEscapeKey` hook added this pass. Not merged: they serve genuinely different navigation trees (one page's admin sections vs. the whole app's overflow menu) and only `TavernMobileMenu` needs the theme-variant branching, so a shared drawer shell would still need a content-slot prop and an optional-variant prop — the overlay/backdrop markup is the only truly identical part.
- *Evidence:* `src/components/admin/AdminNav.tsx` (drawer JSX, line 193), `src/components/tavern/TavernMobileMenu.tsx`.
- *Scope:* maintainability only — no behavior difference either drawer exhibits today.

**Three distinct destructive-action confirmation patterns (#123)** — not fixed, and not unified: (1) plain browser `window.confirm(...)`, used at ~10 call sites across `campaigns/page.tsx`, `story/page.tsx`, `friends/page.tsx`, `admin/page.tsx`, `NotesPanel.tsx`, `TurnTracker.tsx`, and `LoreManagerPanel.tsx`; (2) `settings/page.tsx`'s account deletion, which requires typing the literal fixed phrase `DELETE MY ACCOUNT` into a text field before the button enables; (3) `DataAdvancedPanel.tsx`'s campaign deletion, which reveals a text field requiring the campaign's own title (not a fixed phrase) before the button enables. All three are real, deliberate escalations in friction proportional to the action's severity (a scene reset vs. an entire account vs. an entire campaign's data) rather than accidental drift — but they are three different mechanisms with no shared component, so a fourth destructive action has no obvious pattern to copy without reading existing call sites first.
- *Evidence:* `src/app/campaigns/page.tsx` (line 70), `src/app/settings/page.tsx` (`deleteConfirmation`, line 41), `src/components/admin/DataAdvancedPanel.tsx` (`showDeleteConfirm`/`confirmText`, line 57).
- *Scope:* maintainability only — no action intended without a product decision on which friction level new destructive actions should default to.

**Dead `openPalette`/`closePalette` no-ops in `CommandPaletteContext.tsx` (#124)** — both are declared, memoized with `useCallback`, and exposed on the context value, but each body is only a comment (`// Palette opens via keyboard shortcut, managed internally by CommandPalette component`) and neither has a single caller anywhere in `src/`. The palette's actual open/close state lives entirely inside `CommandPalette.tsx` itself. Not removed here: deleting them is a real API change to `useCommandPalette()`'s return shape, and this pass's mandate was preserving existing behavior, not pruning an unused public surface — even one with no runtime effect to preserve.
- *Evidence:* `src/contexts/CommandPaletteContext.tsx` (lines 25–31); confirmed zero references to either name outside this one file.
- *Scope:* dead code, no behavior risk in removing it — a candidate for a small standalone cleanup rather than folding into this pass.
