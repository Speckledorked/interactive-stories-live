...
describe('persistWorldEvents', () => {
  ...
  it('does nothing for an empty batch', async () => {
    const result = await persistWorldEvents('campaign-1', simTurn(5), [])
  ...
  it('maps tick-origin changes to actorType SYSTEM', async () => {
    ...
    await persistWorldEvents('campaign-1', simTurn(5), [makeChange({ origin: undefined })])
  ...
  it('maps consequence-origin changes to actorType PLAYER', async () => {
    vi.mocked(prisma.worldEvent.createManyAndReturn).mockResolvedValueOnce([{ id: 'e1', significant: true }] as any)
    await persistWorldEvents('campaign-1', simTurn(5), [makeChange({ origin: 'consequence' })])
    const call = vi.mocked(prisma.worldEvent.createManyAndReturn).mock.calls[0][0] as any
  ...
  it('builds a filterable type key from entityType + field', async () => {
    vi.mocked(prisma.worldEvent.createManyAndReturn).mockResolvedValueOnce([{ id: 'e1', significant: true }] as any)
    await persistWorldEvents('campaign-1', simTurn(5), [makeChange({ entityType: 'NPC', field: 'currentPlan' })])
    const call = vi.mocked(prisma.worldEvent.createManyAndReturn).mock.calls[0][0] as any
  ...
  it('stringifies numeric previous/new values', async () => {
    vi.mocked(prisma.worldEvent.createManyAndReturn).mockResolvedValueOnce([{ id: 'e1', significant: true }] as any)
    await persistWorldEvents('campaign-1', simTurn(5), [makeChange({ previousValue: 50, newValue: 47 })])
    const call = vi.mocked(prisma.worldEvent.createManyAndReturn).mock.calls[0][0] as any
  ...
  it('passes wakeSourceType through for a wake-origin change', async () => {
    vi.mocked(prisma.worldEvent.createManyAndReturn).mockResolvedValueOnce([{ id: 'e1', significant: true }] as any)
    await persistWorldEvents('campaign-1', simTurn(5), [makeChange({ origin: 'wake', wakeSourceType: 'FACTION_DEFAULT' })])
    const call = vi.mocked(prisma.worldEvent.createManyAndReturn).mock.calls[0][0] as any
  ...
  it('defaults wakeSourceType to null when absent', async () => {
    vi.mocked(prisma.worldEvent.createManyAndReturn).mockResolvedValueOnce([{ id: 'e1', significant: true }] as any)
    await persistWorldEvents('campaign-1', simTurn(5), [makeChange({ origin: 'tick' })])
    const call = vi.mocked(prisma.worldEvent.createManyAndReturn).mock.calls[0][0] as any
  ...
  it('persists every change, not just significant ones', async () => {
    ...
    await persistWorldEvents('campaign-1', simTurn(5), [
  ...
  it('returns an empty result and swallows errors instead of throwing', async () => {
    ...
    const result = await persistWorldEvents('campaign-1', simTurn(5), [makeChange()])
    expect(result).toEqual({ count: 0, events: [] })
  })
})
...
describe('worldEventDedupeKeys (#377)', () => {
  it('gives the same change list the same keys on a replay', () => {
    ...
    expect(worldEventDedupeKeys(simTurn(5), changes)).toEqual(worldEventDedupeKeys(simTurn(5), changes))
  })
  ...
  it('distinguishes two genuinely different writes to the same field in one turn', () => {
    ...
    const [a, b] = worldEventDedupeKeys(simTurn(5), [
  ...
  it('separates the same change in different turns', () => {
    const [turn5] = worldEventDedupeKeys(simTurn(5), [makeChange()])
    const [turn6] = worldEventDedupeKeys(simTurn(6), [makeChange()])

    expect(turn5).not.toBe(turn6)
  })

  it('separates changes that differ only in their before/after values', () => {
    const [down] = worldEventDedupeKeys(simTurn(5), [makeChange({ previousValue: 50, newValue: 47 })])
    const [up] = worldEventDedupeKeys(simTurn(5), [makeChange({ previousValue: 47, newValue: 50 })])
...
describe('persistWorldEvents — replay safety (#377)', () => {
  ...
  it('writes a dedupeKey per row and asks the database to skip duplicates', async () => {
    ...
    await persistWorldEvents('campaign-1', simTurn(5), changes)

    const call = vi.mocked(prisma.worldEvent.createManyAndReturn).mock.calls[0][0] as any
    expect(call.skipDuplicates).toBe(true)
    expect(call.data[0].dedupeKey).toBe(worldEventDedupeKeys(simTurn(5), changes)[0])
  })
  ...
  it('does not read back when every row was inserted', async () => {
    ...
    await persistWorldEvents('campaign-1', simTurn(5), [makeChange()])
