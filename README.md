...
- [x] **Downtime activity costs were cosmetic** — `ai-downtime-service.ts` computed a gold cost and the UI showed "Cost: X gold" as a real charge, but nothing ever deducted it (`// Note: Character gold tracking removed... For now, we assume the activity is affordable`). `createDynamicActivity` now checks the character's actual gold, rejects the activity if unaffordable, and deducts it on commit — mirroring the `Math.max(0, ...)` clamp pattern already used for scene-driven `resource_changes.gold_delta`
