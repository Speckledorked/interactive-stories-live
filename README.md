...
- [ ] Offscreen background ticks can't yet *introduce* a new hidden location the way they already can for NPCs/factions — `callAIForWorldTurn`'s response schema has no `location_changes` (the offscreen prompt is never asked for them), so there's nothing for `worldTurn.ts` to forward yet. The scene-resolution path is unaffected: it already creates/updates locations with correct `sceneOrigin` gating. Extending the offscreen schema is a small follow-up, not a blocker
