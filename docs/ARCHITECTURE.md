...
- API route test coverage is targeted, not broad — 104 routes, 57
  ...
  checkout, stuck-scene recovery, campaign-scoped user blocking). ~47
...
2. **Keep broadening API route test coverage.** #93, #134, and #135 have
   ...
   safety/moderation, scene-action, social/notification, and world-info
   (world-event log, rumors, wiki index, typing indicator, character
   archetypes) tiers (see the Scorecard row); ~47 routes remain untested —
   health checks, admin reads, tutorial/lore/wiki CRUD, and internal
   cron/job endpoints, none individually high-stakes enough yet to name
...
- **API route test coverage** — targeted coverage exists for the
  ...
  notification, quests/messages/notes/clocks, and world-info tiers (see
  the Scorecard) — but ~47 lower-risk routes remain untested — ongoing,
  not finished.
