...
| Scene endings — a real narrative close, not a status flip | 4 | `Scene.stakes` (a dormant schema field, previously read once in `memoryRetrieval.ts` but never written anywhere) is now populated at scene creation by a small, separate, fail-open AI call (`generateSceneStakes`) grounded in the scene's own opening text, surfaced under the scene header in the story page UI, and echoed back to the model in `<scene_ending>` when a scene is force-ended so it has something concrete to resolve against rather than an abstract "wrap it up." See the matching Known Bugs fix for the other half — the actual silent-no-narration ending bug. Not a 5 — stakes generation is one-shot at scene creation and never revised mid-scene if the situation shifts. |
...
| `AITransparencyPanel`'s "Narration didn't match every roll" summary divided by `matched + mismatched` only, excluding unreported/ambiguous entries from the denominator — a scene with one unreported outcome and nothing else showed a nonsensical "0/0 matched" instead of the real ratio. Fixed: the denominator now sums all four adherence categories. | AI transparency / UI | Minor | Fixed |
| `<response_format>`'s example JSON never included `outcome_echo`, even though the field was explained in prose elsewhere (`<mechanical_outcomes>`) — a field only described in prose but never demonstrated in the literal example template is reliably dropped by the model. Real responses consistently omitted it, permanently starving the outcome-adherence metric of data. Fixed: the field now appears in the actual example object, and `outcomeEchoRepair.ts` backfills any still-unreported entries after the fact (see the Scorecard's outcome-band adherence row). | AI response validation | Major | Fixed |
| `tokenBudget.ts`'s Tier 2 trimming step truncated `current_scene_intro` from the START, but `sceneResolutionRequest.ts` places the most recent exchange at the END of that string — so trimming under budget discarded the freshest continuity first, not the oldest, and the narrator would repeat/"double" beats it could no longer see were already resolved. Fixed: trimming now preserves the tail via a new `truncateFromStart` helper. | AI prompt / token budget | Major | Fixed |
| `<player_character_control>`'s "IF THE PLAYER SAID IT: quote it exactly" rule, combined with presenting a submitted action as `Name: "quoted text"` in the prompt, gave the model no way to distinguish in-character dialogue from an out-of-character rationale bundled into the same action text (e.g. "I comply... because I'm bored") — the aside was narrated as something the character said aloud. Fixed: actions are no longer quote-wrapped in the prompt, and the rule now explicitly distinguishes actual written dialogue from an out-of-character aside. | AI narration | Minor | Fixed |
| Removing or banning a campaign member never touched their `Character` row — there is no real FK relation between `CampaignMembership` and `Character` for a cascade to ride on, despite a stale code comment claiming one existed. A departed member's still-alive character stayed a required scene participant forever, indefinitely blocking resolution for everyone remaining in the scene. Fixed: `actionSubmission.ts` now filters participants against active `CampaignMembership` rows before gating resolution on them. | Scene resolution | Major | Fixed |
...
| `end-scene/route.ts` called `resolveScene(campaignId, sceneId)` with no `forceResolve` flag. Ending a scene before every player had acted threw inside `resolveScene`, was swallowed by the route's own try/catch ("the admin explicitly wants to end this scene"), and the scene silently flipped to RESOLVED with zero narration — no closing beat, no signal to the model that this was the scene's actual end even on the success path. Fixed: `resolveScene` gained an `isSceneEnding` parameter (mirroring the existing `forceResolve`), threaded through `buildSceneResolutionRequest` into a new `<scene_ending>` prompt section (unconditional "this is the final exchange, resolve now" instruction, matching the hard-requirement shape already used for the pacing/stat_labels fixes above); `end-scene/route.ts` now calls `resolveScene(campaignId, sceneId, true, true)`. | Scene resolution / AI narration | Major | Fixed |
| `time_passage` was an optional field the AI could omit entirely, and the degradation ladder made it worse: even after making the field required, `validateAIResponse`'s `'partial'`/`'emergency'` levels never carried it through at all — a response that degraded for *any* unrelated reason silently lost its `time_passage` too, so world-turn pacing (`hoursSinceWorldTurn`) could stay pinned at 0 for real, actively-played campaigns with no error anywhere. Found live (a campaign's own admin panel showed 0.0h banked across multiple played scenes) — the schema-requirement fix alone didn't resolve it, which is what surfaced the second, deeper bug in the salvage ladder itself. Fixed in two parts: `TimePassageSchema` now requires at least one of `days`/`hours`, and `extractValidTimePassage` salvages it independently on every degraded level, not just the full-validation path. | AI response validation / world pacing | Major | Fixed |
| `integrity-autofix.yml`'s `diagnose-and-fix` job had no `id-token: write` in the workflow's `permissions:` block, so `anthropics/claude-code-action@v1`'s own OIDC token exchange failed outright before the diagnosis step could even start. Fixed: added `id-token: write`. | Integrity Engine / CI | Major | Fixed |
| Once OIDC was fixed, the same `claude-code-action` step's own token exchange reconfigures git's local credential for its own use — displacing the credential `actions/checkout@v4` had set up, so the later `git push` step's ambient credential was no longer valid by the time it ran. Fixed: the push step now explicitly re-asserts a known-good credential (`git remote set-url` with `github.token`) immediately before pushing, instead of depending on ambient state left behind by an unrelated step. | Integrity Engine / CI | Major | Fixed |
| `gh label create` (needed to tag a new auto-fix PR) requires `issues: write` on the workflow token — label CRUD is gated under Issues even for a PR-only workflow, and `pull-requests: write` alone isn't enough. The workflow never granted it, so label creation failed on a 403 — and because that failure was originally swallowed (`2>/dev/null \|\| true`), it looked identical to the harmless "label already exists" case for a full run cycle before the real cause was found. Fixed: added `issues: write`, and stopped suppressing that step's stderr so a genuine failure is visible in the logs going forward. | Integrity Engine / CI | Major | Fixed |
| Even with permissions fixed, the auto-fix label name itself (`integrity-autofix:<checkKey>`) exceeded GitHub's 50-character label-name limit for the longest registered checkKey (`character.resources.reputation.keys.resolve`, 43 characters) — `gh label create` failed with "name is too long." Fixed: shortened both label prefixes (`fix:`/`rev:`), leaving comfortable headroom for every checkKey in `checkKeys.ts`'s closed set. | Integrity Engine / CI | Minor | Fixed |
| The repository never had "Allow GitHub Actions to create and approve pull requests" enabled (Settings → Actions → General) — a gate entirely separate from the workflow's own `permissions:` block and from `pull-requests: write`. `gh pr create` failed outright ("GitHub Actions is not permitted to create or approve pull requests") even after every token-scope and label issue above was fixed. Fixed: enabled in repo settings; no workflow change needed. | Integrity Engine / CI | Major | Fixed |
| `verifyOracleTechnique.ts` checks whether a fix's diff touched a test file of the registered oracle technique — deliberately not trusting the diagnosing agent's own report. A real run correctly determined an already-existing test already proved the fix (and verified that test both failed before and passed after), but never touched that file, so the diff carried no evidence and the check couldn't tell that apart from a diagnosis that skipped proving itself entirely — a genuinely correct fix went unmerged as a result. Fixed: the prompt now explicitly requires touching the proving test file even when it already exists, so the diff always carries the evidence the check looks for. | Integrity Engine / CI | Minor | Fixed |
| `integrity-autofix.yml`'s `diagnose-and-fix` agent step had `--max-turns 30`, which every real run needed close to or above regardless of model (Opus: 26, 28, 34; Sonnet: 28, then one run that hit the cap without finishing, `error_max_turns`). Fixed: raised to 50 for real headroom — still bounded, so a genuinely stuck run still can't run away unbounded. | Integrity Engine / CI | Minor | Fixed |
...
1. **Decide whether to enable `integrity-autofix.yml`'s `schedule` trigger
   for real autopilot.** The prerequisite this item used to be — fire it
   once, manually, against a real, deliberately seeded bug — is now
   satisfied: 12 manual (`workflow_dispatch`) runs, culminating in a real
   defect diagnosed, fixed, verified, and merged with zero human
   involvement in the merge decision (PR #153). See the Scorecard's
   Autonomous code-fix pipeline row for what that run proved, and Known
   Bugs for the 8 pipeline-plumbing defects fixing it surfaced along the
   way. The `schedule` trigger stays commented out for now by deliberate
   choice, not because anything is still broken — cadence and comfort
   level are what's left to decide, not correctness.
