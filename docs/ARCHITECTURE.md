...
- API route test coverage is targeted, not broad — 104 routes, 72
  ...
  checkout, stuck-scene recovery, campaign-scoped user blocking). ~22
...
2. **Keep broadening API route test coverage.** #93, #134, and #135 have
   ...
   backfill), health-check, tutorial, internal cron/job, and lore tiers
   (see the Scorecard row); ~22 routes remain untested — mostly base
   list/create endpoints already covered at the single-resource level,
   plus a handful of maps/downtime/misc routes, none individually
...
- **API route test coverage** — targeted coverage exists for the
  ...
  admin/GM-tools, health-check, tutorial, internal cron/job, and lore tiers (see the Scorecard) — but ~22 lower-risk routes
