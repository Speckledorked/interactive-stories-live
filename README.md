...
- **Lore Import**: feed the AI GM reference material — pasted text, a single page URL, or an entire MediaWiki wiki (Fandom, wiki.gg, ...) crawled via its API — chunked, embedded, and retrieved per scene as canon for the narrator; a "reseed world from lore" pass then rebuilds the world's *structure* (factions, learnable systems, archetypes) from that canon too
...
- [x] **Lore-aware world generation**: creation-time generation necessarily runs before lore can be imported, so a "Reseed world from imported lore" admin action (Admin → Lore) closes the loop: a bounded digest sampled evenly across the whole imported corpus grounds a regeneration pass where canon is the highest authority — factions and learnable systems named in the lore are added *by their canonical names*. Merge is non-destructive on a live campaign: factions/capabilities are additive (existing ones untouched), stat labels and the corruption theme fill in only if absent, origin archetypes are regenerated only while no characters exist, and new secret capabilities respect the shadow-branch invariant in themed campaigns
