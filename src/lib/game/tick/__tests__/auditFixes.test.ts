...
describe('tickFactionRelationships (audit fixes)', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    vi.mocked(prisma.factionTie.findMany).mockResolvedValue([] as any)
  })

  it('records a tie against every partner in one tick, each as its own row', async () => {
    // Three EXPAND factions: A becomes rival with BOTH B and C in one tick.
    //
    // This test was written against a real bug: the writer re-spread each
    // faction's fetched JSON map per pair, so the A-C write clobbered A's
    // B entry. #373 made that failure mode unrepresentable rather than
    // merely fixed — a pair IS a row, so two of A's ties cannot occupy the
    // same write. Kept because the behaviour it asserts (every pair
    // recorded) is still the thing that must be true.
    ...
    const pairs = vi.mocked(prisma.factionTie.upsert).mock.calls.map((call) => {
      const { factionAId, factionBId } = (call[0] as any).where.factionAId_factionBId
      return `${factionAId}:${factionBId}:${(call[0] as any).create.type}`
    })
    // All three pairs among three EXPAND factions, each exactly once and
    // each canonically ordered.
    expect(pairs.sort()).toEqual(['a:b:RIVAL', 'a:c:RIVAL', 'b:c:RIVAL'])
  })

  it('expires a relationship whose other side has collapsed', async () => {
    const survivor = makeFaction('survivor', { goal: 'DEFEND' })

    vi.mocked(prisma.factionTie.findMany).mockResolvedValue(
      factionTieTable([['survivor', 'fallen', 'RIVAL', 2]]) as any
    )
    vi.mocked(prisma.faction.findMany)
    ...
    expect(prisma.factionTie.deleteMany).toHaveBeenCalledWith({
      where: { factionAId: 'fallen', factionBId: 'survivor' },
    })
  ...
  it('writes nothing on the expiry path when dryRun is true', async () => {
    ...
    vi.mocked(prisma.factionTie.findMany).mockResolvedValue(
      factionTieTable([['survivor', 'fallen', 'ALLY', 2]]) as any
    )
    vi.mocked(prisma.faction.findMany)
    ...
    expect(prisma.factionTie.upsert).not.toHaveBeenCalled()
...
describe('tickFactions same-tick absorption (audit fix #199)', () => {
  ...
  it('does not let the absorbing faction\'s own regular-tick write clobber its same-tick absorption transfer', async () => {
    ...
    const a = makeFaction('a', { goal: 'EXPAND', resources: 30, stability: 10, military: 20, ties: { b: { type: 'RIVAL', since: 1 } } })
