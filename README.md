...
| AI response validation | 4 | Improved (`#36`): one bounded repair round-trip is attempted before falling through to the degradation ladder — no longer purely silent. Fixed (`#66`): the background world-turn call is validated too, having previously been a bare `JSON.parse` feeding the same state writer. Still basic JSON mode, not strict structured outputs (`#35`, open — see Known Bugs / Known Issues). |
...
| Cross-system economy (faction wealth ↔ items ↔ downtime ↔ quests) | 2 | Partly fixed (`#76`, `#77`): quest payouts are now real transfers out of `Faction.resources`, and a faction that can't afford its promise defaults on part of it — so player wealth and the faction sim finally move each other. Still a 2: `Debt` carries no roll weight and inventory has no rarity/value/price, both deliberately parked pending a product call (see above). |
...
**The economy, beyond faction wealth (#44, #47)** — the faction-wealth edge is now built (see Shipped), and the rest is deliberately still open. `Debt` carries no roll-time weight the way `Standing` does, and inventory has no rarity/value/price/crafting/trading at any layer. Both are buildable; what's missing is a decision about whether MythOS *has* an economy in the sense players would expect, or deliberately doesn't. Building one incrementally without that call risks a half-economy that's worse than none — so these stay parked on purpose, not by oversight.
...
**P3 cleanup — `#60`, `#61`, `#90`:** the five leftover "GM Notes" labels in the admin edit forms are now "Host Notes", matching the campaign-host reframing shipped everywhere else in that file. `getWorldStateChanges()` is restored as a real accessor (its extraction logic lives in a dependency-free `worldStateChanges.ts` so importing it into a client component can't drag Prisma into the browser bundle). And every confirmed-dead field named in that entry is **wired up rather than dropped** — the first pass deleted three of them, which was the wrong call:
