...
| Friends system | 3 | Real, persisted, two-model design (`FriendRequest`/`Friendship`), a real API surface (`/api/friends`, `/requests`, `/search`) and a real page (`/friends`). Not documented anywhere in this file until now. Not a 4 — never independently adversarially audited (block/unfriend edge cases, duplicate-request races) the way the rest of this Scorecard's 4s were. |
