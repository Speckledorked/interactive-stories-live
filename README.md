...
- [ ] Still dead schema surface: `Session*` models + orphaned transcript-export route, `Campaign.mentionsEnabled`/`soundEffectsEnabled`. Also noted, not yet fixed: `lib/downtime/ai-downtime-service.ts` instantiates its own `PrismaClient()` instead of importing the shared singleton from `lib/prisma` — every other file in the codebase uses the shared client
