...
- API route test coverage is targeted, not broad — 104 routes, 64
  ...
  checkout, stuck-scene recovery, campaign-scoped user blocking). ~40
...
2. **Keep broadening API route test coverage.** #93, #134, and #135 have
   ...
   archetypes), and campaign admin/GM-tools (away-recap, chronicle-share,
   export, integrity, Story Log + its resummarization pass, world-extras
   backfill) tiers (see the Scorecard row); ~40 routes remain untested —
...
- **API route test coverage** — targeted coverage exists for the
  ...
  admin/GM-tools tiers (see the Scorecard) — but ~40 lower-risk routes
