...
### Phase 8 — Tooling & Scale (stretch) ✅ (core; live tick replay deferred)
...
- [ ] "Replay a tick" (the other half of that item) — true replay, re-running tick logic against historical/counterfactual state, is real added scope: every tick handler would need a sandboxed dry-run mode so a replay can't double-write memory/history/wiki entries. Deliberately deferred rather than half-built into a "stretch" item; the tick-log viewer above already answers "why did this happen," which covers the more common actual need
