...
| **API route test coverage** | 3 | Improving (`#95`). Was 6 test files for 93 routes. The first batch went to the claim the product leads with: every entity-list route and the aggregate campaign GET now have behavioural fog-of-war tests — a player is restricted, an admin is not, a non-member is refused *before* the query rather than handed an empty list, and no unrecognised role is ever treated as staff. That is the difference between `#94`'s structural check (the helper is reached) and this (the route behaves): a route can import the helper and still hand it to the wrong query. Mutation-checked against three real leak modes. Still a 3, not a 4: the write paths — `resolve-scene`, `end-scene`, `members`, `invites` — remain untested.|
