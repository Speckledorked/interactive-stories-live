...
| WorldGraph (location adjacency) | 3 | A real, if flat, adjacency graph (`LocationAdjacency`, undirected, distance-weighted) backs `directNeighborsOf`/`shortestPath`/`nearestLocation` (`worldGraph.ts`), and two real consumers already use it instead of a blind pick: `decideTerritoryClaim`'s candidate selection and `decideNpcTick`'s "work" location both prefer a genuine graph neighbor of "home" when adjacency data covers it, falling back to their exact pre-adjacency behavior (alphabetical-first / hash-rotation) otherwise. AI-authored backfill (`worldGraphGenerator.ts`) infers a plausible graph from existing lore for campaigns with none. This row's stated O(V²)-node-selection blocker is fixed (2026-08-13, #259): `shortestPath` now uses a binary min-heap (O((V+E) log V)), verified against a brute-force Bellman-Ford reference on a larger branchy/chorded graph in addition to the existing small-graph tests. Not a 4 yet regardless — per the Scorecard Audit Log rule below, fixing this row's own named blocker in the same pass that names it doesn't itself justify the increase; a later, separate adversarial pass with the explicit goal of finding defects in this exact system, finding none, is what would. |
