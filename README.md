...
**`Location` is stored as free text, not a foreign key (#42)**
- *Why it matters:* weather modifiers, NPC prompt relevance, and split-party grouping all join on the free-text `currentLocation` string; one AI-written variant ("the Docks" vs. "The Docks District") silently decouples all three systems from each other.
- *Evidence:* `Character.currentLocation` / `NPC.currentLocation` (`prisma/schema.prisma`), consumed by `lib/game/tick/weatherTick.ts`, `lib/ai/worldState.ts`, and the location-grouping logic in `story/page.tsx`.
- *Scope:* persistence, core gameplay.
- *Suggested fix:* Add a nullable `Location` FK, backfilled by name once, alongside the existing string.

**Conditions, perks, and earned Abilities are prompt text only — never mechanically enforced (#5, #49, #50, #51)**
- *Why it matters:* condition `mechanicalEffect` text ("Stunned: -1 to all rolls"), perk bonus text ("+1 ongoing…"), and earned Abilities (`organic_advancement.new_moves`) all reach the player and the narrator prompt, but `computeMechanics` never applies any of them — whether they actually happen is delegated entirely to whether the LLM honors its own prose.
- *Evidence:* `lib/game/harm.ts` (`getTotalConditionPenalty`, `canAct` — fully built, zero callers), `lib/game/resolution.ts` (`computeMechanics`), `lib/game/capabilities.ts` (perk bonuses), `lib/game/advancement.ts` (`organic_advancement.new_moves`); resolution only ever rolls the 7 fixed `BASIC_MOVES` regardless.
- *Scope:* core gameplay, AI behavior.
- *Suggested fix:* Wire each into `computeMechanics` as a real modifier (same pattern already used for weather/standing), or reword all three as explicitly narrative-only with no numeric claims.
