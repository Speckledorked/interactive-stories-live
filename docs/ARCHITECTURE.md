...
| System | Score | Status |
|---|:-:|---|
| Server-rolled dice/outcome engine | 4 | `computeMechanics` (`src/lib/game/resolution.ts`) is genuinely pure, RNG-injected, and unit-tested with real edge cases — stacked-modifier totals including a fully-negative case, both dice extremes, and the exact outcome-band boundaries (`resolution.goldenVectors.test.ts`). `resolveActionMechanics`, named alongside it above, is the orchestrator around it and is NOT itself pure (real Prisma reads/writes, a live network call) — the two shouldn't be described as one "pure" unit. Not a 5 — "unconditional, not gated by any setting" is true (no toggle exists) but understates a real, untested single point of failure: the whole pipeline is gated on `classifyActions` succeeding, which hard-requires `OPENAI_API_KEY` and a live OpenAI call (`resolution.ts` — `if (!apiKey) return []`); a missing key or an API outage silently degrades every action that exchange to unrolled freeform narration, with only a `console.error` and no player-facing signal. See #200. |
| Faction simulation (goals/collapse/succession/territory) | 4 | Goal-driven stat deltas (`decideFactionTick`), banded reassessment (`decideFactionGoalReassessment`, now also overridable by a drifted `beliefVector` — see the Cultural drift row), collapse (`decideFactionCollapse`) → absorption or remnant succession, territory claims. `decideSuccession` is a standalone, tested, pure function with deterministic tie-breaking (`compareCandidates`) that holds even on a full tie (falls through to `id.localeCompare` as a final backstop) and a correctly-handled zero-candidate case (`decideSuccession` returns `null`, not a throw). Not a 5 — a real, silent same-tick bug: `tickFactions` loops over a faction array snapshotted before the loop starts, so when faction A is absorbed into faction B mid-tick and B is processed later in that same loop, B's own regular tick write (still using the stale pre-absorption snapshot) silently overwrites the just-transferred resources back to their pre-absorption values. Untested, undisclosed until this audit — see #199. |
...
| World tick orchestration | 4 | 19 deterministic handlers confirmed (`runWorldTick`/`TICK_HANDLERS`), sequenced same-tick dependencies with genuinely broad pairwise-ordering coverage (10+ documented dependencies, not just a couple of pairs), zero AI calls confirmed by direct audit across every handler, all inside one `prisma.$transaction` — a failed turn rolls back cleanly instead of committing partial state. Not a 5 — `TICK_TRANSACTION_TIMEOUT_MS` (20s) is a flat constant, not scaled to the admin-configurable `factionCap`/`npcCap` (`resolveTickCaps` has no upper clamp on either); a campaign that raises those caps has no matching timeout headroom, so a large enough campaign could hit the ceiling and abort the whole turn. Fails safe, but undisclosed — see #203. |
| Debt economy | 4 | Directional, persisted, and consumed as a real roll modifier (`debtModifier`), correctly clamped to ±2 regardless of how many debts exist — not a label. Not a 5 — the underlying query that feeds it has no row limit; a debt-heavy campaign sees unbounded row/prompt growth even though the mechanical outcome stays safely clamped. See #221. |
| Faction standing | 3 | Feeds `computeMechanics()` directly via a banded modifier, correctly zeroed for a collapsed/absorbed faction. Not a 4 — the modifier's LOW-band cap is keyed on `Faction.influence`, which is never written by any tick or consequence path (only at world-generation and via manual admin edit) — the exact "bled dry by a lost war" scenario the code's own comment describes cannot occur through simulation. See #218. |
| Relationships (trust/tension/respect/fear) | 4 | Feeds `computeMechanics()` via a banded `relationshipModifier`. Deliberately never rendered to players as raw numbers. Not a 5 — only 3 of the 4 listed axes actually feed the modifier; `fear` is deliberately excluded (the classifier doesn't currently signal which direction it should cut) but the row's own framing implies all 4 are mechanical. See #220. |
...
| Character harm/death state machine | 3 | Full model: auto-conditions (`applyHarm`), death saves (`makeDeathSave`), permanent injury (`performRecoveryRoll`), a `canAct()` gate, one parse boundary for the harm blob (`parseHarmState`). Three recovery speeds — medical attention (`applyMedicalAttention`), in-game time (`accrueNaturalRecovery`), and rest (`applyRest`) — all blocked by recurring-harm conditions (`blocksNaturalRecovery`). Not a 4 — two real gaps: nothing in the consequence pipeline checks `isAlive` before applying harm/corruption/conditions to a character, so a dead character can still have state mutated; and the roll that actually decides whether a character crosses into Taken Out uses raw `Math.random()` inline, not the injectable RNG the rest of the dice engine uses — untestable without globally mocking `Math.random`. See #213. |
...
| Consequence engine (player action → faction/NPC state) | 3 | Deterministic per-action deltas (`extractAndApplyConsequences`/`applyConsequences`). Entity lookup now goes through the same roster-based `resolveEntityByNameOrId` every other AI write-back applier uses (fetched once per batch, not once per name) — see the Fix Log. Not a 4 — the exact-name-match branch uses `.find()` and never checks for duplicates, unlike the fuzzy-match path (which correctly returns `ambiguous`); a campaign with two same-named entities (a real, confirmed-possible state — see the Integrity Engine's own duplicate-names check) can have a consequence silently applied to the wrong one. See #215. |
| Character progression (advancement) | 3 | Usage-gated growth with real PbtA constraint validation. AI-authored perks/Abilities carry a real per-arc grant budget (`countGrantsInArc`, applied in `applyOrganicGrowth`), not a level-up button. Not a 4 — the budget check is a read-then-write race outside the main `pc_changes` transaction, with no row lock or optimistic-concurrency check: two overlapping scene resolutions on the same character can both read the same stale grant count, both see budget available, and the second write silently clobbers the first — able to exceed the per-arc cap or lose a grant. See #214. |
...
| Memory retrieval (RAG) | 3 | Genuine pgvector cosine search, cost-tracked, gracefully returning empty on a campaign with no memories yet. Not a 4 — "bounding table growth" doesn't hold uniformly: consolidation only ever touches `MINOR`/`NORMAL` memories ≥20 turns old; `MAJOR`/`CRITICAL` rows are permanently exempt and grow forever on an active campaign, and the consolidation code's own header comment already admits the table "is never capped at write time." See #216. |
...
| AI response validation | 4 | One bounded repair round-trip (`validateAIResponseWithRepair`), then a degradation ladder (`extractValidWorldUpdates`) that salvages `world_updates` through the real schemas section-by-section and element-by-element rather than zeroing the whole thing. `time_passage` is now a required field (`TimePassageSchema`'s own `.refine()` demands at least `days` or `hours`, even if the value is 0) rather than silently omittable, and — since making it required alone didn't survive a degraded response — `extractValidTimePassage` salvages it independently on the `'partial'`/`'emergency'` levels too, so world-turn pacing (`hoursSinceWorldTurn`) can no longer starve just because some other part of a response needed to degrade. See the Fix Log. The background world-turn call is validated too (`callAIForWorldTurn` → `validateWorldTurnResponse`), not a bare parse. Basic JSON mode, not strict structured outputs — a deliberate decision (2026-08-02) to keep this contract rather than switch. This repair/degradation rigor is NOT uniform across every AI call site, though — `worldGenerator.ts`'s campaign-creation call silently coerces malformed faction fields to plausible defaults rather than rejecting them (permanent world state from unvalidated data), and several other generators (`worldGraphGenerator.ts`, `consequenceExtraction.ts`, `chronicleNarration.ts`, `sceneStakes.ts`) use bare `JSON.parse` with no schema and no repair round-trip. See #217. |
| Clock advancement (non-ambition clocks) | 3 | Deterministic, faction/relation-driven pacing (`decideClockAdvancement`), not a random coin flip — every write path correctly clamps at `maxTicks`, and a clock linked to a collapsed faction stalls/defaults correctly rather than crashing. Not a 4 — `advanceClocks` runs OUTSIDE the main world-tick transaction, using the bare `prisma` singleton in an un-transacted per-clock loop, called after `runWorldTick`'s `$transaction` has already committed. `worldTurn.ts`'s own code comment already admits this ("a transaction can't cover" clocks) — a mid-loop failure leaves some clocks advanced and others stale against already-committed world state, with no repair path. See #229. |
| Quest lifecycle | 3 | A structured reward grant is applied deterministically the first time a quest completes. FAILED/ABANDONED are not inert — walking away costs trust and standing, failing honestly costs respect (`applyQuestFailureCost`). An unresolved quest-giver (`resolveQuestGiver` returning unresolved, guarded by `hasQuestGiver`) costs nothing rather than guessing. Not a 4 — "the first time" isn't actually enforced: `justCompleted` is a read-then-write check with no unique constraint, version column, or row lock, the same unguarded pattern the code's own comment already admits exists for quest *creation*; two racing scene resolutions on the same quest can both pass the check and double-grant rewards. See #212. |
| Combat / complex exchange resolution | 3 | Conflicting actions on the same target are ranked by actual roll outcome (`rankActionsByOutcome`/`compareActionsByOutcome`, surfaced via `detectConflicts`), not left to an AI punt. No dedicated combat subsystem beyond PbtA resolution — by design. Not a 4 — the tie-break relies on an unstable secondary sort: the actions being compared come from a query ordered only by `actionPriority` with no secondary key, so Postgres gives no ordering guarantee when priority AND outcome AND total all tie, contradicting the "deterministic tiebreak" framing. See #219. |
| Inventory / items | 4 | Structured `armorValue`/`damageBonus`/`effect`, with `'heal'` enforced deterministically (`resolveConsumableHeal`, correctly floors healing at 0 harm). `value`/`rarity` under a per-arc budget for grants that go through it. NPCs have a lighter mirrored harm model (`healHarm`). Still JSON-blob CRUD, not a relational Item table; no merchant/trading layer by design. A prior `slots` capacity field was removed and never re-enforced — no cap on total item count today. See #222. |
| Downtime activities | 2 | A deterministic, risk-weighted outcome category (`decideDowntimeDayEvent`) is rolled before the AI narrates, replacing a random coin flip. Completion rewards are genuinely applied (`applyDowntimeRewards`), not generated and discarded. Downgraded — a real, currently-exploitable gap: nothing prevents a character from starting unlimited concurrent downtime activities (no active-activity check, no unique constraint on `(characterId, status)`), and each one's rewards bypass the per-arc rarity budget AND the faction-affordability check quest rewards enforce. Combined, this is an unbudgeted item/gold farming vector, not just a documentation gap. See #211. |
...
| DB `Move` table | 4 | The 7 fixed basic moves stay the mechanical source of truth (`BASIC_MOVES`), confirmed never editable per-campaign — `Move.rollType` is written but never read back for mechanics. Flavor-text fallback is genuinely tested for the failure modes `generateMoveFlavor` itself can produce (null return, per-band omission). Not a 5 — the fallback expression only optional-chains `moveFlavor`, not `.outcomes`; a malformed `Move.outcomes` (reachable via `campaign-exporter.ts`'s `importMoves`, which writes untrusted import data with zero validation) throws, and the outer catch then drops dice mechanics for the *whole exchange*, not just that move's flavor — contradicting the "safe, graceful" framing. Untested. See #201. |
...
| Downtime completion rewards | 2 | Parsed strictly (unparseable entries skipped and logged, never guessed). Downgraded — "applied through the same primitives quest payouts use" overstates it: `applyDowntimeRewards` calls `mergeGrantedItems` directly, never through `applyGrantBudget` (the per-arc rarity cap quest rewards use), and invokes no faction-affordability check at all — see #211, the same finding as the Downtime activities row above; these two rows describe one combined exploit. |
...
| World history as a decision input | 4 | Crisis targeting (`pickMostThreateningFaction`) reads a genuinely bounded, windowed slice of `WorldEvent` history, not the whole campaign. The war-outcome cooldown is real and exactly deterministic — `WAR_EXHAUSTION_TURNS=6` vs. `WAR_DEFEAT_EXHAUSTION_TURNS=12`, precisely 2x, not merely "roughly," and thoroughly unit-tested. Not a 5 — the doc's "faction goal-commitment... reads back structured history" claim undersold a real risk: `factionTick.ts`'s goal-commitment lookback query has **no `take` limit**, fetching a campaign's entire goal-change history every single tick, with no pruning/archival job for `WorldEvent` anywhere in the codebase. Unbounded, compounding query growth on a long-running campaign. See #202. |
...
| Cross-system economy (faction wealth ↔ items ↔ downtime ↔ quests) | 3 | Quest/downtime payouts are real transfers out of a faction's resources (`assessPayout`); a broke faction pays partially and defaults on the rest (though see the Downtime rows above — this doesn't apply to downtime rewards). Debt moves the dice in both directions. Granted items merge through one genuinely shared path (`mergeGrantedItems`). Not a 4 — `clampGoldDelta` only clamps the delta's magnitude; the actual "gold can't go negative" guarantee is a `Math.max(0, ...)` floor duplicated at 3 separate call sites rather than centralized in `clampGoldDelta` itself — currently correct everywhere, but fragile against a future 4th call site. See #223. |
...
| Fog-of-war enforcement mechanism | 4 | One shared `visibleTo(model, role)` gate, correctly handling the polarity difference (clocks gate on hidden state, everything else on discovered state). An unknown role fails closed, tested. The exemption list is narrow and genuinely self-policing — 2 entries, each restricted to `select: { id: true }` only, with its own staleness test. Not a 5 — the "structural test" that's supposed to catch a bypass is regex pattern-matching (`DIRECT_READ = /prisma\.(nPC|faction|location|clock)\.(findMany|findFirst|findUnique)/g`), not real AST analysis — unlike this same codebase's own `entityResolutionConvention.test.ts`, which genuinely walks the TypeScript AST for an analogous guard. The regex approach could miss computed/bracket property access, a model reference held behind a helper, or query methods outside the pattern list (e.g. `aggregate`/`groupBy`). See #205. |
...
| Environmental state & location aging | 4 | `Location.conditionScore` (0-100, DB-range-checked) drifts from war/contested-territory presence (`decideConditionDrift`) and derives a closed condition-tag vocabulary on read (`deriveConditionTags`: `RUINED`/`DAMAGED`/`STABLE`/`PROSPEROUS`/`CONTESTED`/`ABANDONED`) rather than a free-form label. Feeds migration's own distress signal directly (see the next row), and — corrected here, since this row previously undersold it — also feeds a real roll modifier: `locationConditionPenalty` (`resolution.ts`) shifts rolls ±1 for RUINED/ABANDONED vs. PROSPEROUS locations, wired into `computeMechanics` as `siteConditionMod` and populated from a real per-character location lookup in production. Not a 5 — only the roll-modifier half exists; there's still no access *gate* on condition the way `checkCorruptionGate` gates quest acquisition/location entry on corruption. See #206. |
