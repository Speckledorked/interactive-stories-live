...
export async function enhanceAIPromptWithPbtA(
  ...
  if (sceneId) {
    ...
    if (scene?.stakes) {
      pbtaContext.sceneStakes = scene.stakes
    }
  }

  // Get overall threat level from clocks (check for filled/nearly filled clocks)
  const allThreateningClocks = await prisma.clock.findMany({
    where: {
      campaignId,
      category: 'threat',
    },
  ...
  // Count clocks that are at least 75% full
  const threateningClocks = allThreateningClocks.filter(
    clock => clock.currentTicks >= (clock.maxTicks * 0.75)
  ).length
