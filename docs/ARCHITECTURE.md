...
| Resource infrastructure & logistics | 3 | A location with `resourceSlots` only pays out to its owning faction while at least one `SupplyRoute` touching it is unblockaded (`decideExtraction`); an ESCALATING war over a contested location automatically blockades routes through it and lifts the blockade the same turn the war resolves. Corrected 2026-08-13 (adversarial audit): this row's own text previously said routes were "not yet validated against real spatial adjacency... not higher than a 3 until routes are generated from (or checked against) `WorldGraph` adjacency" — that bar was already met by the code, not still open. `decideSupplyRouteCreation` connects an unrouted resource location to the nearest OTHER location its faction owns via the real adjacency graph (`worldGraph.ts`'s `nearestLocation`), falling back to an arbitrary-but-deterministic pick only when no adjacency data covers it — the doc's own scoring criterion for a higher score, already satisfied, just never re-scored. Re-auditing this row (the actual goal of this pass, not just confirming that fix) found a real, previously-unknown, un-triggered-in-any-test defect instead: `decideSupplyRouteCreation` only ever checked a location's connection state against the routes that existed BEFORE the pass started, so two resource locations owned by the same faction that are each other's nearest neighbor, both starting with no route at all, each independently decided to connect to the other — two `SupplyRoute` rows for the same pair, created in the same tick. Fixed with a same-tick `connectedThisPass` tracking set (same shape as `factionTick.ts`'s #199 same-tick-consistency fix) — see the Fix Log. Because this pass found and fixed a new defect rather than confirming zero, it does not itself justify moving the score past 3 per the Scorecard Audit Log rule below; a later, separate clean pass is what would. |
